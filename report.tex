...
\begin{longtable}{L{1.3cm} L{2.4cm} L{2.8cm} L{2.5cm} L{4cm}}
\toprule
...
LM-2 & Select a Sport          & SportManager, SportFactory & \texttt{selectSport()}, \texttt{createSport()}   & Creates the selected sport implementation through the factory. \\[0.4em]
LM-3 & Generate a League       & Sport, League           & \texttt{createLeague()}, \texttt{addTeam()}, \texttt{generateFixtures()} & Creates the league structure and generates the fixture schedule. \\[0.4em]
LM-4 & Generate Teams, Players, and Coaches & Sport, Team & \texttt{createTeam()}, \texttt{createPlayer()}, \texttt{addPlayer()} & Creates teams and populates them with players and coaches. \\[0.4em]
...
LM-7 & View the League Table   & SportManager, League    & \texttt{showLeagueTable()}, \texttt{getTable()}  & Displays the current league standings. \\[0.4em]
LM-8 & Update League Standings & League, Sport           & \texttt{updateStandings(result)}, \texttt{calculatePoints()} & Updates standings and points after each match. \\[0.4em]
LM-9 & Finish a Season         & League, GameSession     & \texttt{advanceWeek()}, \texttt{getMatchesOfWeek()} & Season completion is inferred when all fixtures are played. \\[0.4em]
...
TM-2 & View the Team Schedule & SportManager, League       & \texttt{showFixture()}, \texttt{getMatchesOfWeek()} & Displays the team's upcoming and past matches. \\[0.4em]
TM-3 & Set Lineup Before a Match & Team, Lineup            & \texttt{setLineup()}, \texttt{isValid()} & Defines and validates the starting lineup and substitutes. \\[0.4em]
...
\midrule
\endhead
MS-1 & Simulate Matches Automatically & SportManager, Match & \texttt{startMatch()}, \texttt{start()}, \texttt{play()}, \texttt{finish()} & Runs the automated match simulation. \\[0.4em]
...
\midrule
\endhead
IM-1 & Track Injuries by Number of Games & Sport, InjuryRecord, Player & \texttt{applyInjuries()}, \texttt{decrementGames()}, \texttt{recover()} & Tracks injuries based on the number of matches missed. \\[0.4em]
IM-2 & Prevent Injured Players from Being Selected & Player, Team & \texttt{isAvailable()}, \texttt{getAvailablePlayers()} & Ensures injured players cannot be selected for matches. \\
\bottomrule
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.8\textwidth]{screen-sport-selection.png}
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}
          \textit{[Screenshot — Sport Selection Screen]}\vspace{4cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.8\textwidth]{screen-team-selection.png}
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}
          \textit{[Screenshot — Team Selection Screen]}\vspace{4cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.9\textwidth]{screen-dashboard.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{4.5cm}
          \textit{[Screenshot — Dashboard Screen]}\vspace{4.5cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.9\textwidth]{screen-fixture.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{4cm}
          \textit{[Screenshot — Fixture Screen]}\vspace{4cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.9\textwidth]{screen-squad.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{4.5cm}
          \textit{[Screenshot — Squad Screen]}\vspace{4.5cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.9\textwidth]{screen-match.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{4.5cm}
          \textit{[Screenshot — Match Screen]}\vspace{4.5cm}}}
...
\begin{figure}[H]
    \centering
    % Replace with: \includegraphics[width=0.9\textwidth]{screen-league-table.png}
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{4cm}
          \textit{[Screenshot — League Table Screen]}\vspace{4cm}}}
