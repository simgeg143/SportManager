...
public final class SportManager {
    ...
    /** @return total number of matchdays in the season. */
    public int getTotalWeeks()    { return session.getLeague() != null
                                        ? session.getLeague().getTotalRounds() : 0; }
    ...
    /**
     * Returns all matches across the entire season, grouped by week.
     * Called by FixtureController. (TM-2)
     */
    public List<List<Match>> showFixtureData() {
        if (session.getLeague() == null) return List.of();
        League league = session.getLeague();
        java.util.List<java.util.List<Match>> all = new java.util.ArrayList<>();
        for (int i = 0; i < league.getTotalRounds(); i++) {
            all.add(league.getRoundMatches(i));
        }
        return all;
    }
